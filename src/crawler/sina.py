...
#counting
c.execute("SELECT COUNT(*) FROM tweets WHERE post_datetime > %s", (datetime.combine(date.now() - timedelta(days = 1), time()),))
t = c.fetchone()
if t == None:
	count = 0
else:
...
c.execute("SELECT COUNT(*) FROM tweets WHERE post_datetime > %s", (datetime.combine(date.now() - timedelta(days = 1), time()) - timedelta(days = datetime.now().isoweekday() - 1),))
t = c.fetchone()
if t == None:
	count = 0
else:
