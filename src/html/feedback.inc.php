...
function get_feedbacks($num, $page)
{
    ...
    connect_db();
    $view = "SELECT $select from feedback, (SELECT nickname, user_id FROM userinfo) as ui WHERE ui.user_id = feedback.user_id OR ISNULL(feedback.user_id) ORDER BY post_datetime DESC$limit";
